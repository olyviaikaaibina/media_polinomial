{{-- resources/views/halamanguru.blade.php --}}
@extends('layout.navbar')

@section('title', 'Halaman Guru - Login')

@section('content')

    <style>
        .guru-login-page {
            background-color: #FDFDE8 !important;
            min-height: calc(100vh - 160px);
            padding: 40px 10px 50px;
            display: flex;
            align-items: center;
        }

        .guru-login-container {
            max-width: 1180px;
            margin: 0 auto;
            width: 100%;
        }

        .guru-login-card {
            background-color: #D0DDD0;
            border-radius: 20px;
            padding: 32px 32px 28px;
            box-shadow: 0 20px 40px rgba(15, 23, 42, 0.12);
            border: 1px solid rgba(148, 163, 184, 0.25);
            width: 100%;
            max-width: 460px;
        }

        .guru-login-title {
            font-size: 1.4rem;
            font-weight: 700;
            color: #111827;
            margin-bottom: 18px;
        }

        .guru-label {
            font-size: 0.85rem;
            font-weight: 500;
            color: #4b5563;
            margin-bottom: 4px;
        }

        .guru-input {
            width: 100%;
            border-radius: 8px;
            border: 1px solid #d1d5db;
            height: 42px;
            padding: 8px 12px;
            font-size: 0.9rem;
            color: #111827;
            background-color: #ffffff;
        }

        .guru-input::placeholder {
            color: #9ca3af;
        }

        .guru-input:focus {
            outline: none;
            border-color: #2563eb;
            box-shadow: 0 0 0 3px rgba(37, 99, 235, 0.25);
        }

        /* ⛔ Hilangkan ikon password bawaan browser (biar tidak muncul mata kedua) */
        input[type="password"]::-ms-reveal,
        input[type="password"]::-ms-clear {
            display: none !important;
        }

        input[type="password"]::-webkit-textfield-decoration-container {
            display: none !important;
        }
    </style>

    <div class="guru-login-page">
        <div class="guru-login-container">
            <div class="guru-register-container">
                @if (session('success'))
                    <div class="alert alert-success">
                        {{ session('success') }}
                    </div>
                @endif

                @if ($errors->any())
                    <div class="alert alert-danger">
                        <ul class="mb-0" style="list-style: none; padding-left: 0;">
                            @foreach ($errors->all() as $error)
                                <li>{{ $error }}</li>
                            @endforeach
                        </ul>
                    </div>
                @endif


                <div class="row align-items-center g-4">

                    {{-- Kiri: Ilustrasi --}}
                    <div class="col-lg-6 text-center">
                        <img src="{{ asset('img/3.png') }}" alt="Ilustrasi Login" style="max-width: 70%; height: auto;">
                    </div>

                    {{-- Kanan: Card Login --}}
                    <div class="col-lg-6 d-flex justify-content-center">
                        <div class="guru-login-card">

                            <h1 class="guru-login-title">Masuk untuk melanjutkan</h1>

                            <form action="{{ route('guru.login.store') }}" method="POST">
                                @csrf

                                {{-- Email --}}
                                <div class="mb-3">
                                    <label class="guru-label" for="email">Email</label>
                                    <input type="email" id="email" name="email" class="guru-input"
                                        placeholder="Masukkan email Anda">
                                </div>

                                {{-- Password --}}
                                <div class="mb-3">
                                    <label class="guru-label" for="password">Password</label>

                                    <div class="position-relative">

                                        {{-- Input Password --}}
                                        <input type="password" id="password" name="password" class="guru-input"
                                            placeholder="Masukkan password Anda" style="padding-right: 45px;">

                                        {{-- Icon Mata --}}
                                        <span id="togglePassword" style="
                                                position: absolute;
                                                right: 12px;
                                                top: 50%;
                                                transform: translateY(-50%);
                                                cursor: pointer;
                                                color: #6b7280;
                                                padding-left: 8px;
                                                border-left: 1px solid #d1d5db;
                                                height: 60%;
                                                display: flex;
                                                align-items: center;
                                            ">
                                            <svg id="eyeIcon" width="20" height="20" viewBox="0 0 24 24" fill="none"
                                                stroke="#6b7280" stroke-width="2" stroke-linecap="round"
                                                stroke-linejoin="round">
                                                <path d="M1 12s4-7 11-7 11 7 11 7-4 7-11 7S1 12 1 12z"></path>
                                                <circle cx="12" cy="12" r="3"></circle>
                                            </svg>
                                        </span>

                                    </div>
                                </div>

                                {{-- Ingat Saya + Button --}}
                                <div class="d-flex justify-content-between align-items-center mb-1">
                                    <label style="font-size: 0.85rem; color:#4b5563;">
                                        <input type="checkbox" name="remember" style="margin-right: 6px;">
                                        Ingat Saya
                                    </label>

                                    <button type="submit" class="guru-login-btn"
                                        style="background-color:#A0937D; border:none; color:white; padding:8px 26px; border-radius:999px;">
                                        Masuk
                                    </button>
                                </div>

                                {{-- Buat akun --}}
                                <div class="guru-bottom-text"
                                    style="margin-top: 18px; text-align:center; font-size:0.85rem; color:#4b5563;">
                                    Belum punya akun?
                                    <a href="{{ route('guru.register') }}" style="color:#16a34a;">Buat akun sekarang!</a>
                                </div>

                            </form>

                        </div>
                    </div>

                </div>
            </div>
        </div>

        {{-- JS Toggle Password --}}
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                const toggle = document.getElementById('togglePassword');
                const pwd = document.getElementById('password');
                const eyeIcon = document.getElementById('eyeIcon');

                toggle.addEventListener('click', function () {
                    const isPassword = pwd.type === "password";
                    pwd.type = isPassword ? "text" : "password";

                    // Ganti icon jika ingin (mata terbuka -> tertutup)
                    eyeIcon.innerHTML = isPassword
                        ? `<path d="M17.94 17.94A10.94 10.94 0 0 1 12 20c-7 0-11-8-11-8a21.81 21.81 0 0 1 5.06-6.06M9.9 4.24A10.94 10.94 0 0 1 12 4c7 0 11 8 11 8a21.81 21.81 0 0 1-2.41 3.6M1 1l22 22"></path>`
                        : `<path d="M1 12s4-7 11-7 11 7 11 7-4 7-11 7S1 12 1 12z"></path><circle cx="12" cy="12" r="3"></circle>`;
                });
            });
        </script>

@endsection