...
@section('content')
    ...
    <style>
        ...
        #p5ContohInteraktif1 canvas {
            max-width: 100%;
            height: auto !important;
        }

        /* ====== TEMPAT EMBED P5 (MARI MENCOBA) ====== */
        .p5-card-wrap {
            border-radius: 16px;
            border: 1px solid rgba(0, 0, 0, .10);
            background: #fff;
        ...
        /* Canvas p5 harus nurut ukuran container */
        #p5Latihan1 canvas {
            display: block;
            max-width: 100%;
            height: auto !important;
    ...
    <div class="materi-wrap">
        ...
        <div class="card card-explore">
            ...
            <p>
                ...
                memengaruhi luas genangan $f(x)$. Kamu diminta membaca “peta” genangan dari sebuah fungsi.
            </p>
            ...
            <p class="note">
                ...
                <b>$$f(x)=x^2-2x-3$$</b>
                <br>
                Tugasmu: <b>geser waktu</b>, amati nilai fungsi, dan lihat bentuk grafiknya.
            </p>
                ...
                <div class="mission-row">
                    <div class="panel">
                        <div class="panel-title">
                            ...
                            <span class="chip" id="expInsightChip">Insight: siap!</span>
                        </div>

                        <input class="range" id="expX" type="range" min="-6" max="6" step="1" value="0">

                        <div class="small-note">
                            Geser slider untuk mengubah $x$. Nilai $f(x)$ berubah dan titik pada grafik ikut bergerak.
                        </div>
                        ...
                        <div class="kpi">
                            ...
                            <div class="box">
                                <div class="lbl">Luas Genangan</div>
                        ...
                        <div class="exp-explainbox" id="expExplain">
                            <div class="head">Penjelasan Saat Ini</div>
                            ...
                            <div class="line">
                                ...
                                <div class="txt" id="txtIdea">
                                    Jika suatu saat $f(x)=0$, maka titik tepat di sumbu-x (titik potong sumbu-x).
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        {{-- =========================
        CONT0H INTERAKTIF (tetap)
        ========================= --}}
        <div class="card card-example">

            <div id="p5ContohInteraktif1"></div>

            <p class="note" style="margin-top:14px;">
                <b>Tips:</b> titik oranye = titik penting (potong sumbu & vertex). Titik hijau = titik pilihan.
            </p>
        </div>

        {{-- =========================
        CARD MARI MENCOBA (BARU)
        ========================= --}}
        <div class="card card-example">

            <div class="p5-card-wrap">
                <div id="p5MariMencoba1"></div>
            </div>

            <p class="note" style="margin-top:14px;">
                <b>Catatan:</b> Kurva disembunyikan dulu sampai tombol <b>Plot ke Diagram</b> ditekan.
            </p>
        </div>
    ...
    <script>
        (function () {
            ...
            function fx(x) { return x * x - 2 * x - 3; }
            ...
            function drawGraph(x0) {
                ...
                if (0 >= yMin && 0 <= yMax) { ctx.beginPath(); ctx.moveTo(pad, oy); ctx.lineTo(W - pad, oy); ctx.stroke(); }
                if (0 >= xMin && 0 <= xMax) { ctx.beginPath(); ctx.moveTo(ox, pad); ctx.lineTo(ox, H - pad); ctx.stroke(); }
                ...
                ctx.beginPath(); ctx.moveTo(p0.sx, pad); ctx.lineTo(p0.sx, H - pad); ctx.stroke();
                ctx.beginPath(); ctx.moveTo(pad, p0.sy); ctx.lineTo(W - pad, p0.sy); ctx.stroke();

                ctx.fillStyle = 'rgba(27,122,42,.95)';
                ctx.beginPath(); ctx.arc(p0.sx, p0.sy, 6, 0, Math.PI * 2); ctx.fill();
                ctx.strokeStyle = 'rgba(255,255,255,.95)';
                ctx.lineWidth = 2;
                ctx.beginPath(); ctx.arc(p0.sx, p0.sy, 9, 0, Math.PI * 2); ctx.stroke();
            }

            const SPECIAL = { vertex: { x: 1, y: -4 } };
            ...
            function updateExplanation(x, y, trend) {
                if (txtSub) txtSub.innerHTML = `Masukkan $x=${x}$: $f(${x})=${x}^2-2(${x})-3=${y}$.`;
                if (txtPoint) txtPoint.innerHTML = `Titik saat ini: $(${x},${y})$.`;

                let posisi = (y > 0) ? `Titik <b>di atas</b> sumbu-x (positif).`
                    : (y < 0) ? `Titik <b>di bawah</b> sumbu-x (negatif).`
                        : `Titik tepat <b>di sumbu-x</b> (akar).`;

                let arah = (trend === 'Naik') ? `Nilai $f(x)$ <b>naik</b>.`
                    : (trend === 'Turun') ? `Nilai $f(x)$ <b>turun</b>.`
                        : (trend === 'Tetap') ? `Nilai $f(x)$ <b>tetap</b>.` : '';

                if (txtPos) txtPos.innerHTML = `${posisi} ${arah}`;

                const isVertex = (x === SPECIAL.vertex.x && y === SPECIAL.vertex.y);
                if (txtIdea) {
                    if (y === 0) txtIdea.innerHTML = `✅ Ini <b>titik potong sumbu-x</b> karena $f(${x})=0$.`;
                    else if (isVertex) txtIdea.innerHTML = `⭐ Ini <b>titik puncak (minimum)</b> yaitu $(1,-4)$.`;
                    else if (x < 1) txtIdea.innerHTML = `Mendekati $x=1$, nilai $f(x)$ cenderung <b>mengecil</b>.`;
                    else txtIdea.innerHTML = `Lewat $x=1$, nilai $f(x)$ cenderung <b>membesar</b>.`;
                }

                if (y === 0) setInsight('ok', 'Insight: titik potong sumbu-x');
                else if (isVertex) setInsight('ok', 'Insight: titik puncak');
                else setInsight('warn', 'Insight: amati perubahan');
            ...
            function updateExplorerUI() {
                ...
                if (lastX !== null) {
                    const dy = y - fx(lastX);
                    trend = dy > 0 ? "Naik" : dy < 0 ? "Turun" : "Tetap";
                }
            ...
            if (slider && canvas && ctx) {
                window.addEventListener('resize', () => { resizeCanvas(); updateExplorerUI(); });
                slider.addEventListener('input', updateExplorerUI);
                requestAnimationFrame(() => { resizeCanvas(); updateExplorerUI(); });
            }
    ...
    {{-- LOAD FILE P5 --}}
    <script defer src="{{ asset('js/contohinteraktif1.js') }}"></script>
    <script defer src="{{ asset('js/marimencoba1.js') }}"></script>
    <!-- panggil sketch setelah p5 -->
